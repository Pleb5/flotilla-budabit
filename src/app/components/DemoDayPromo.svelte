<script lang="ts">
  ...
  $effect(() => {
    ...
    const demoDayEvents = calendarEvents.filter(e => {
      if (
        (getTagValue("title", e.tags) || "").toLowerCase().includes("budabit demo day") &&
...
<div class="card2">
  <div class="flex flex-col items-center gap-y-4">
    <h1 class="flex items-center justify-center gap-2 font-semibold max-sm:text-2xl sm:text-3xl">
      BudaBit Demo Day
    </h1>
    <h1 class="flex gap-2 text-center font-semibold max-sm:text-xl sm:text-2xl">
      Show off Your Work in Freedom Tech!
    </h1>
    {#if nextDemoDay}
      <CalendarEventItem {url} event={nextDemoDay} />
    {/if}
    <Button class="btn btn-primary btn-lg md:btn-wide" onclick={apply}>Apply Now</Button>
  </div>
</div>
