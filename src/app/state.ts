...
export const derivePatchGraph = (addressA: string) =>
  withGetter(derived(
    deriveEvents(repository, { filters: [{ kinds: [1617], "#a": [addressA] }] }),
...
export const deriveIssueThread = (rootId: string) =>
  withGetter(derived(
    ...
    ([$root, $comments, $statuses]) => (
      $root ? assembleIssueThread({ root: $root as unknown as any, comments: $comments as unknown as any[], statuses: $statuses as unknown as any[] }) : undefined
    ),
  ))
...
export const deriveStatus = (rootId: string, rootAuthor: string, euc: string) =>
  withGetter(derived([deriveIssueThread(rootId), deriveRepoGroup(euc)], ([$thread, $group]) => {
...
export const deriveEffectiveLabels = (eventId: string) =>
  withGetter(derived(
    ...
    ([$evt, $external]) => {
      ...
      const t = (($evt as any).tags || []).filter((t: string[]) => t[0] === "t").map((t: string[]) => t[1]);
 ...
 */
export const loadRepoContext = (args: { addressA?: string; rootId?: string; euc?: string; relays?: string[] }) => {
  const { filters } = buildRepoSubscriptions({ addressA: args.addressA, rootEventId: args.rootId, euc: args.euc });
  const defaults = (GIT_RELAYS && GIT_RELAYS.length > 0) ? GIT_RELAYS : INDEXER_RELAYS;
  const relays = (args.relays || defaults).map(u => normalizeRelayUrl(u)).filter(Boolean) as string[];
