...
/** Creation/recovery metadata belongs to its approved destination, not the open route. */
export function createRepoCreationPublisher({
  ownerPubkey,
  getActivePubkey,
  transport,
}: {
  ...
  return async (event, context) => {
    ...
    const assertCurrent = () => {
      ...
      if (!repoAddress?.startsWith(prefix) || !repoAddress.slice(prefix.length))
        throw new Error("Repository publication requires the approved destination coordinate")
      ...
      if (
        ...
        throw new Error("Repository metadata does not match the approved destination coordinate")
    ...
    return transport.publish(event, relays, {
      repoAddress,
      assertCurrent,
...
export const createForkRepoPublisher = createRepoCreationPublisher
