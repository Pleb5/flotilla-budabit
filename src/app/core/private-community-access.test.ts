...
describe("private access lifecycle", () => {
  ...
  it("denial after ACK disposes the socket; explicit retry authenticates a new socket and refetches history", async () => {
    const {control, reads, socket} = setup()
    await control.start()
    ...
    expect(connection?._disposed).toBe(true)
    expect(control.sockets.has(relay)).toBe(false)
    await control.start()
    expect(socket).toHaveBeenCalledTimes(2)
    expect(control.sockets.get(relay)).not.toBe(connection)
  ...
  it("exposes degraded per-relay status and clears data on revoked connection", async () => {
    ...
    reads[1].onEose!(second)
    expect(get(control.view).access).toBe("partial") // CLOSED cannot become complete through a late callback
