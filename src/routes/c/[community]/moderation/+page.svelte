<script lang="ts">
  ...
  const missingFormSections = $derived(grantableSections.filter(item => !$activeCommunityAdmissionForms[item.section.name]))
  ...
  const selectedDraft = $derived(
    selected
      ? getDraft(selected.section.name, activeForm)
      : undefined,
  )
  const selectedDraftErrors = $derived(selectedDraft ? validateAdmissionFormDraft(selectedDraft) : [])

  const setDraft = (sectionName: string, draft: CommunityAdmissionFormDraft) => {
    drafts = {...drafts, [sectionName]: draft}
  }

  const updateDraft = (sectionName: string, updater: (draft: CommunityAdmissionFormDraft) => void) => {
  ...
  const updateQuestion = (
    ...
    updateDraft(selected.section.name, draft => {
      draft.questions = draft.questions.map(question => question.id === questionId ? updater(question) : question)
    })
  }
  ...
  const setQuestionType = (questionId: string, type: CommunityAdmissionQuestionType) => {
    updateQuestion(questionId, question => ({
      ...question,
      type,
      options: type === "singleChoice" || type === "multipleChoice" ? question.options.length ? question.options : defaultOptions() : [],
    }))
  }
  ...
  const addOtherOption = (questionId: string) => {
    updateQuestion(questionId, question => {
      ...
      return {...question, options: [...question.options, {id: "other", label: "Other", isOther: true}]}
    })
  }
    ...
    <section class="card2 bg-alt col-4 flex flex-col gap-3 p-4 shadow-md">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p class="text-sm opacity-70">Create one active application form per section you can grant.</p>
        </div>
        <Button class="btn btn-ghost" onclick={() => (pageMode = "queue")}>Back to review queue</Button>
      </div>
    ...
    <div class="col-4 grid gap-4 lg:grid-cols-[minmax(220px,280px)_1fr]">
      ...
      {#if selected && selectedDraft}
        <div class="grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(320px,420px)]">
          <form bind:this={formBuilderElement} class="card2 bg-alt flex flex-col gap-4 p-4 shadow-md" onsubmit={preventDefault(publishSelectedForm)}>
            ...
            {#if selectedDraftErrors.length > 0}
              <div class="rounded-box border border-warning bg-warning/10 p-3 text-sm text-warning">
              ...
              <Field>
                ...
                {#snippet input()}
                  <input
                    ...
                    oninput={event => updateDraft(selected.section.name, draft => (draft.name = event.currentTarget.value))} />
                {/snippet}
              </Field>
              <Field>
                ...
                {#snippet input()}
                  <textarea
                    ...
                    oninput={event => updateDraft(selected.section.name, draft => (draft.description = event.currentTarget.value))}></textarea>
                {/snippet}
              </Field>
            </div>

            <div class="flex flex-wrap gap-2">
              <Button class="btn btn-sm" onclick={() => addQuestion("shortAnswer")}>Add short answer</Button>
              <Button class="btn btn-sm" onclick={() => addQuestion("paragraph")}>Add paragraph</Button>
              <Button class="btn btn-sm" onclick={() => addQuestion("singleChoice")}>Add single choice</Button>
              <Button class="btn btn-sm" onclick={() => addQuestion("multipleChoice")}>Add multiple choice</Button>
            </div>
            ...
            <div class="flex flex-col gap-3">
              {#each selectedDraft.questions as question, index (question.id)}
                <article class="rounded-box border border-base-300 bg-base-100 p-4 shadow-sm">
                  <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
                    <div class="flex flex-wrap items-center gap-2">
                      <Button class="btn btn-xs" disabled={index === 0} onclick={() => moveQuestion(question.id, -1)}>Up</Button>
                      <Button class="btn btn-xs" disabled={index === selectedDraft.questions.length - 1} onclick={() => moveQuestion(question.id, 1)}>Down</Button>
                      <label class="flex items-center gap-2 rounded-full bg-base-200 px-3 py-1 text-sm">
                        <input
                          type="checkbox"
                          class="checkbox checkbox-xs"
                          checked={question.required}
                          onchange={event => updateQuestion(question.id, current => ({...current, required: event.currentTarget.checked}))} />
                        Required
                      </label>
                    </div>
                    <Button class="btn btn-error btn-xs" onclick={() => removeQuestion(question.id)}>Delete</Button>
                  </div>
                  ...
                  <div class="grid gap-3 md:grid-cols-[1fr_180px]">
                    <input
                      ...
                      oninput={event => updateQuestion(question.id, current => ({...current, label: event.currentTarget.value}))} />
                    <select
                      class="select select-bordered w-full"
                      value={question.type}
                      onchange={event => setQuestionType(question.id, event.currentTarget.value as CommunityAdmissionQuestionType)}>
                  ...
                  <div class="mt-4">
                    {#if question.type === "shortAnswer"}
                      <input class="input input-bordered w-full" disabled placeholder="Type your answer here" />
                    {:else if question.type === "paragraph"}
                      <textarea class="textarea textarea-bordered min-h-24 w-full" disabled placeholder="Type your answer here"></textarea>
                    {:else}
                        ...
                        {#each question.options as option}
                          <div class="flex items-center gap-2">
                            ...
                            <input
                              ...
                              oninput={event => updateOption(question.id, option.id, event.currentTarget.value)} />
                            <Button class="btn btn-ghost btn-xs" disabled={question.options.length <= 2} onclick={() => removeOption(question.id, option.id)}>Remove</Button>
                          </div>
                        {/each}
                        <div class="flex flex-wrap items-center gap-2 pt-1">
                          <Button class="btn btn-outline btn-sm" onclick={() => addOption(question.id)}>Add option</Button>
                          {#if !question.options.some(option => option.isOther)}
                            <span class="text-sm opacity-60">or</span>
                            <Button class="btn btn-outline btn-sm" onclick={() => addOtherOption(question.id)}>add other</Button>
                          {/if}
                        </div>
                      </div>
                    {/if}
              ...
              {:else}
                <p class="rounded-box bg-base-200 p-4 text-center text-sm opacity-70">Add at least one question to publish this form.</p>
              {/each}
            </div>

            <div class="flex flex-col items-end gap-2 border-t border-base-300 pt-4">
              <Button type="submit" class="btn btn-primary" disabled={selectedDraftErrors.length > 0}>Publish form</Button>
              <p class="text-right text-xs opacity-60">Last step: publish this form to community relays.</p>
            </div>
          </form>

          <aside class="card2 bg-alt flex flex-col gap-4 p-4 shadow-md xl:sticky xl:top-4 xl:self-start">
            <div>
              <h2 class="text-xl font-semibold">Preview</h2>
              <p class="text-sm opacity-70">Applicants will see this form before submitting a public application.</p>
            </div>
              ...
              <div class="mt-4 flex flex-col gap-3">
                {#each selectedDraft.questions as question}
                  <div class="rounded-box border border-base-300 p-3">
                    ...
                    {#if question.type === "shortAnswer"}
                      <input class="input input-bordered input-sm w-full" disabled placeholder="Type your answer here" />
                    {:else if question.type === "paragraph"}
                      <textarea class="textarea textarea-bordered min-h-20 w-full" disabled placeholder="Type your answer here"></textarea>
  ...
  {:else}
    <section
      ...
      <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <Button class={`btn ${setupComplete ? "btn-primary" : "btn-warning"}`} onclick={() => (pageMode = "forms")}>Edit forms</Button>
      </div>
    </section>
