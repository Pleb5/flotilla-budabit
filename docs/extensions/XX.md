...
> ℹ️ **Flotilla Host Implementation Notes**
>
> Flotilla consumes kind 30033 events as described below with the following runtime behavior:
>
> - `identifier` is pulled from the `d` tag (fallback to event id) and drives the host’s settings key.
> - `widgetType` is derived from the `l` tag (defaults to `basic` if missing/unknown).
> - The first `button` tag of type `app` determines the iframe `appUrl` for `action`/`tool` widgets. Missing `appUrl` causes install failure for those types.
> - Icon origin is inferred from `appUrl`/`icon`/`image` to validate `postMessage` origins.
> - Permission tags (`permission` / `perm`) are mapped to the same `nostr:*` / `ui:*` namespace used by legacy manifests; Flotilla denies privileged bridge requests unless explicitly present.
> - `basic` widgets render through host-managed DOM (no iframe). `action`/`tool` widgets spawn sandboxed iframes with `allow-scripts allow-same-origin`.
>
> For a runtime overview and install flows, see [`docs/extensions/README.md`](./README.md).
