# Legacy Repository Import Architecture (Dormant)

> Historical description of `useImportRepo.svelte.ts`, which is no longer exposed by the application. Some transaction/recovery statements below describe intended contracts rather than durable collaboration-event guarantees. Do not use this as the current product specification or enable the old hook. See [Initial Repository Import](./import-repo-architecture.md) for the supported flow.
