...
GRASP and Nostr Git paths are compiled in unless `FEATURE_GRASP=0`. Experimental CI/CD hooks require `FEATURE_CICD=1`. The alternative history importer is disabled in all builds and cannot be enabled by `FEATURE_IMPORT_REPO`.
...
```
Root Application
├── @welshman/* (vendored workspace) - Core Nostr functionality
...
Smart Widget protocol through the same extension bridge.
