...
`activeExactCommunityRelays` remains bootstrap-capable and is used for loading and live reads. `activeExactCommunityPublishRelays` is definition-only and is the route/component source for community-bound writes. If the active definition is unavailable or declares no valid relays, publication is unavailable even when route hints or discovery relays were sufficient to load cached community state. Explicit definition-based publishers use `getCommunityScopedPublishRelays` with the same rule. Community-star controls require explicit definition-derived write destinations.
...
| Admission review reaction                  |               `7` | Scoped community relays only. | Grants, rejections, and revocations belong to the exact community's admission workflow. Applicant outboxes and app/indexer relays are not publication targets. |
...
Widget association cleanup is calculated per old association: a kind-5 deletion uses only the definition-declared relays of the communities that association targets. Neither new target selections, widget hosting/provenance, association hints, nor user outboxes expand those destinations. All cleanup destinations are validated before any deletion starts; missing definitions or empty declared relay sets block cleanup.

Applicant notification discovery retains per-account exact community coordinates and relay hints locally before submitting an application. Known memberships and locally available authored applications also seed this retained context, which survives refresh, rejection, submission deletion, and loss of membership. These hints bootstrap the exact signed definition; addressed outcome queries then run on its declared community relays, followed by authority/evidence validation. Existing addressed outcome queries on user/app relays remain read-only compatibility for older broadly published decisions. This local context is not cross-device synchronization: a fresh installation needs to rediscover the application/community context.
...
Repository collections use kind-7 stars. Personal selection publishes an ordinary repository star to repository-declared relays. Community selection publishes a separate targeted star to repository-declared relays and its kind-30222 association to the selected community's definition-declared relays. Selecting both performs both operations independently. Star deletion uses repository relays; association deletion uses the association's community relays. Repository cards and details share `RepoCollectButton`, including existing selections and removals. This is a destination/curation model, not encrypted bookmark storage. Retries preserve the original publication destinations (or only their failed subset).

| Event                            |                        Kind | Publish relays                                                                                                                                                                                                                        | Why                                                                                                         |
| -------------------------------- | --------------------------: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| Repo announcement                |                     `30617` | At least one valid relay declared by the announcement is required first. After that circuit breaker passes, Git indexers, user outbox relays, explicit GRASP targets, and the one directly bound community's accepted relays may be added. | Repo announcements are broad discovery records and use exactly one `h=<communityId>` for current community binding; no repository targeting wrapper is published. |
...
Inbound collaboration events are matched independently from this strict write policy. An issue, pull request, pull request update, or related activity belongs to a repository when any valid declared repository coordinate matches the active repository or its rename history. Additional valid coordinates may intentionally target forks or other repositories and must not suppress the event. Budabit-authored collaboration events still emit exactly one canonical repository coordinate; maintainers and recipients use `p` tags rather than additional repository coordinates. Repository announcements and state remain singular addressable objects.
