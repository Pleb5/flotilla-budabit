...
if [ $EXIT_CODE -eq 0 ]; then
  ...
  if [ -n "$HIVE_CI_WORKFLOW" ]; then
    ...
    sudo act -v -P ubuntu-latest=catthehacker/ubuntu:act-latest -W "$HIVE_CI_WORKFLOW" 2>&1 | tee "$ACT_LOG_FILE" || ACT_EXIT=$?
  else
    echo "Running all workflows"
    sudo act -v -P ubuntu-latest=catthehacker/ubuntu:act-latest 2>&1 | tee "$ACT_LOG_FILE" || ACT_EXIT=$?
  fi
