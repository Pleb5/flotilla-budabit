...
describe("clone-url-fallback utilities", () => {
  ...
  describe("filterValidCloneUrls", () => {
    it("filters out nostr:// pseudo URLs", () => {
      ...
      expect(result).toEqual(["https://github.com/user/repo.git", "git@github.com:user/repo.git"])
    })
    ...
    it("filters out empty strings and whitespace", () => {
      ...
      const result = filterValidCloneUrls(urls)
      expect(result).toEqual(["https://github.com/repo.git", "git://other.com/repo"])
    })
  ...
  describe("withUrlFallback", () => {
    it("returns first successful result", async () => {
      ...
      const result = await withUrlFallback(["https://url1.com", "https://url2.com"], operation)
    ...
    it("updates cache on success", async () => {
      ...
      await withUrlFallback(["https://failed.com", "https://success.com"], operation, {
        repoId: "test-repo",
      })
    ...
    it("reuses the active fallback on subsequent reads", async () => {
      const urls = ["https://primary.example", "https://secondary.example", "https://third.example"]
      ...
      expect(secondOperation).toHaveBeenCalledWith("https://secondary.example")
    })
  ...
  describe("withMultiWrite", () => {
    ...
    it("reports total failure", async () => {
      ...
      const result = await withMultiWrite(["https://url1.com", "https://url2.com"], operation)
  ...
  describe("getCloneUrlsFromEvent", () => {
    ...
    it("extracts clone URLs from event tags", () => {
      ...
      expect(urls).toEqual(["https://github.com/user/repo.git", "git@github.com:user/repo.git"])
    })
    ...
    it("handles multiple clone tags", () => {
      ...
      expect(urls).toEqual(["https://url1.com/repo.git", "https://url2.com/repo.git"])
    })
  ...
  describe("convenience wrappers", () => {
    it("cloneWithFallback wraps withUrlFallback", async () => {
      ...
      const result = await cloneWithFallback(["https://url.com"], cloneFn, "test-repo")
    ...
    it("pushToAllRemotes wraps withMultiWrite", async () => {
      ...
      const result = await pushToAllRemotes(["https://url1.com", "https://url2.com"], pushFn)
