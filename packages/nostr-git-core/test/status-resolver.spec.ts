...
describe("resolveStatus", () => {
  ...
  it("accepts maintainer status but not synthetic root author authority", () => {
    const events: LocalStatusEvent[] = [
  ...
  it("never grants authority from an imported tag, on native or imported roots", () => {
    for (const importedRoot of [true, false]) {
      expect(
        resolveStatus({
          statuses: [makeStatus("forged", 1632, "root", 1000, [["imported", ""]])],
          rootAuthor: "root",
          maintainers: new Set(),
          importedRoot,
        }).final,
      ...
      expect(
        resolveStatus({
          statuses: [makeStatus("owner", 1632, "owner", 1000, [["imported", ""]])],
          rootAuthor: "root",
          repoOwner: "owner",
          maintainers: new Set(),
          importedRoot,
