...
function isRelayPublishError(value: string): boolean {
  return /^(?:auth-required|blocked|denied|error|failed|failure|invalid|mute|rate-limited|rejected|restricted|unsupported)(?::|\b)/i.test(
    value.trim()
  );
}

export async function publishWidget(options: PublishOptions): Promise<void> {
  ...
  if (
    options.uploadFirst &&
    (options.blossomServers?.length || (options.githubRepo && options.githubTag))
  ) {
    ...
    try {
      ...
      if (options.blossomServers && options.blossomServers.length > 0) {
        const sk = skHex ? hexToBytes(skHex) : undefined;
        ...
        if (uploadedArtifactUrls.length === 0) {
          throw new Error('Blossom upload did not return a URL');
        }
      }
      ...
      if (uploadedArtifactUrls.length === 0 && options.githubRepo && options.githubTag) {
        ...
        if (!token) {
          ...
          if (parts.length !== 2 || !parts[0] || !parts[1]) {
            throw new Error(
              `Invalid github-repo format: "${options.githubRepo}". Expected format: owner/repo`
            );
          }
      ...
      if (uploadedArtifactUrls.length === 0) {
        throw new Error('Artifact upload did not return a URL');
      }
      ...
      throw new Error('Pre-signing artifact upload failed');
    }
  }
  ...
  if (relays.length === 0) {
    throw new Error(
      'No community relays supplied. Publish through BudaBit or pass the tagged community relays with --relay.'
    );
  }
  ...
  if (bunkerUrl) {
    ...
    if (nip46ClientForSigning) {
      ...
      signedEvent = await nip46ClientForSigning.signEvent(
        unsignedEvent as import('nostr-tools').UnsignedEvent
      );
      nip46Client = nip46ClientForSigning;
    } else {
  ...
  try {
    ...
    results.forEach((result, index) => {
      ...
      if (!relay) return;
      if (result.status === 'fulfilled' && !isRelayPublishError(result.value)) {
    ...
    if (successful.length === 0) {
      throw new Error('Failed to publish widget to any relay');
    }
  } finally {
    pool.close(relays);
...
program
  .name('publish-widget')
  .description(
    'Sign and publish a Smart Widget (kind 30033) event to Nostr relays. Supports local signing, NIP-46 remote signing, and artifact uploads to Blossom/GitHub.'
  )
  .requiredOption(
    '--event <path>',
    'Path to the unsigned event.json file',
    'dist/widget/event.json'
  )
  .option('--relay <url...>', 'Tagged community relay URLs (required)', [])
