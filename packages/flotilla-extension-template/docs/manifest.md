...
Declare each community shared config consumed by the widget. Namespace and key use the same host identifier rules: 1-120 characters, beginning with an alphanumeric character and containing only alphanumerics, `:`, `.`, `_`, or `-`.

- **Example**: `["shared-config", "budabit-calendar-widget", "featured-calendar-event"]`
- Explicit declarations are authoritative for shared-config slot recovery. Legacy manifests with no declarations retain identifier-based matching.
...
- `--shared-config "namespace=key"` (repeatable; emits one `shared-config` tag per declaration)
- Shared-config consumers must also declare the `community:querySharedConfig` permission.
