...
interface WidgetInitPayload {
  ...
  /** Optional additive host API support catalog */
...
Use `getHostActionSupport(payload, action)` or `getHostFeatureSupport(payload, feature)` from `@budabit/ext-shared` before relying on newer host behavior. Both helpers return `"unknown"` when an older host omits the catalog, rather than incorrectly treating that host as unsupported.
