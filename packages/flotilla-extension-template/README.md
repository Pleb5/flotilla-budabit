...
This directory is tracked source in the Budabit monorepo, not a Git submodule.
Its SDK, bridge, tooling and template app share the root pnpm 10.12.4 workspace.
...
### Develop the in-tree template

### 1) Install

```bash
# From the Budabit root (Node 22, pnpm 10.12.4)
...
The remaining package-specific commands below run from this directory, or through
`pnpm --filter budabit-extension-template run <script>` at the Budabit root.
...
pnpm manifest:generate \
  ...
  --shared-config 'budabit-calendar-widget=featured-calendar-event' \
...
- Repeat `--shared-config namespace=key` for each community shared config the widget consumes. These declarations make host slot recovery matching explicit and authoritative.
- `--permissions` should include `nostr:subscribe` for real-time subscriptions and `community:querySharedConfig` when declaring or reading shared configs.
