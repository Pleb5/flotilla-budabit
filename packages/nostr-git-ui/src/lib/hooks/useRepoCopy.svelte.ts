...
/** Shared Git copy engine. Nostr forks and anonymous public sources have explicit policies. */
export function useRepoCopy(options: UseForkRepoOptions = {}) {
  ...
  async function forkRepository(
    ...
    try {
      ...
      const publicSource = originalRepo.publicSource;
      if (publicSource && (originalRepo.sourceAnnouncementEvent || config.includeBranches?.length))
        throw new Error(
      ...
      const repoDescription = sameLogicalRepo
        ...
        : (config.description ??
          originalRepo.description ??
          (publicSource ? "" : `Fork of ${originalRepo.owner}/${originalRepo.name}`));

      const sourceUrlCandidates = publicSource
        ? [publicSource.cloneUrl]
        : buildSourceCloneCandidates(...(originalRepo.cloneUrls || []));
      if (sourceUrlCandidates.length === 0) {
        throw new Error("No valid source clone URLs available for duplication");
      }
      const upstreams = publicSource
        ? undefined
      ...
      transactionJournal = new RepoCreationTransactionJournal({
        ...
        operation: publicSource ? "import" : "fork",
        ownerPubkey: userPubkey,
        repoName: forkName,
      ...
      // Snapshot public refs before any remote mutation; only heads/tags within browser limits.
      const publicRefs = publicSource
        ? await readPublicCopyRefs(gitWorkerApi, publicSource, abortSignal)
        : undefined;
      ...
      const announcementAdmission: RepoSyncAnnouncementAdmission =
        sameLogicalRepo && selectedGraspRelays.length === 0
          ...
          : await publishRepoSyncAnnouncement({
              ...
              upstreams: sameLogicalRepo ? undefined : upstreams,
      ...
      updateProgress("validate", "Validating repository copy configuration...", "running");
      updateProgress(
        "validate",
      ...
      for (let index = 0; index < sourceUrlCandidates.length; index++) {
        ...
        const sourceToken = publicSource
          ? undefined
          : getSourceTokenForUrl(sourceUrl, availableTokens);
        try {
          updateProgress(
            "fork",
          ...
          if (publicSource) {
            const statuses = await operationSession!.waitForTrackedOperations();
            if (!statuses.length || statuses.some((status) => status.state !== "completed"))
              throw new Error(
                "Full source clone completion could not be confirmed; no Git push was started"
              );
          }
      ...
      if (publicSource && publicRefs)
        await verifyPublicCopyRefs(
          gitWorkerApi,
          localRepoId,
          publicSource,
          publicRefs,
      ...
      remotePushResults = await syncLocalRepoToTargets({
        ...
        strictSourceSnapshot: Boolean(publicSource),
        targets: selectedTargets,
        userPubkey,
