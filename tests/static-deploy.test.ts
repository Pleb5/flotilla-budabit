...
describe("ordered static deployment", () => {
  ...
  it("emits the same ordering for the real lftp path", async () => {
    ...
    expect(stdout.indexOf(".budabit-rename-probe")).toBeLessThan(immutable)
    expect(stdout.indexOf("# Pass 0:")).toBeLessThan(immutable)
    expect(stdout.match(/\.budabit-upload/g)).toHaveLength(8)
    expect(stdout.match(/^mv /gm)).toHaveLength(5)
